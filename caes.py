...
The loading functionality is implemented via a Reader class and a method 'load()'. The method reades a text file and follows a sequence of steps to first
split the file and then load its content in a caes. The first step consists of splitting the file into its main components: Arguments, Proof-standards and
Assumptions. Consequently, each section is processed further tokenize and disambiguate the variuos parts and finally everything is loaded in a caes case.
---------------------------------------------------------------------------------------------------------------------------------------------------------
"""
...
class Reader(object):
    ...
    """
    ----------------------------------------------------------------
    Function that maps a list of strings to their propositional form.
    ----------------------------------------------------------------
    When a '-' symbol is encountered a negation of the 
    proposition is given
    """
    def argToProp(self, propList):
        pList = list()
        for prop in propList:
            if prop[0] == '-':
                prop = prop[1:]
                pList.append(PropLiteral(prop).negate())
            else:
                pList.append(PropLiteral(prop))
        return pList
    ...
    """
    ------------------------------------------------
    Function that loads the file specified as a caes
    ------------------------------------------------
    NOTE: the Regular Expression library is used throughout the function, the library provides efficent pattern matching algorithms that are fundamental
          for the parsing reasoning implemented below
    """
    def load(self, initFile):
        self.fileInstance = initFile

        """ some of the arguments needed to initialize the caes class are instantiated since 
            argset, assumptions, audience and proof standards are preferred to be globally visible in the method.
            'weights' is a dictionary that will be loaded with the value of each argument name and their respective weight"""
        argset = ArgumentSet()
        assumptions = {}
        weights = {}
        audience = Audience({},{})
        ps = ProofStandard([(PropLiteral("x"), "beyond_reasonable_doubt")])
        caes = CAES(argset, audience, ps)


        """ a first split of the file into the three main sections, that is:
            arguments, proofstandards and assumptions"""
        section = re.findall('{.*?}', self.fileInstance.read(), re.DOTALL)
        if (len(section)!=3):
            ...
            """ extracts the arguments lines from argument section"""
            argLines = re.split('\\n', section[0]) 
            argLines = list(filter(lambda x: re.match('[\s+{}]', x)==None and x!="", argLines))

            """ in each loop an argument line is processed, the argument name and weight at first and the conclusions, premises and exceptions afterwards"""
            for i in range(len(argLines)):
                info = re.split(':', argLines[i])

                if (len(info)!=2):
                    print('\nERROR: wrong syntax for arguments in input text, wrong use of \':\'')
                    print(i+1,'argument wasn\'t loaded!\n')
                else:
                    """separates the argument name and weight from the current line being processed and loads those in the 'weight' dictionary"""
                    name_weight = re.split('\(', info[0])
                    argName = re.findall('\w*', name_weight[0])[0]
                    weightString = re.findall('[0-9]*\.[0-9]*', name_weight[1])
                    if(weightString!=[]):
                        argWeight = float(weightString[0])
                    else:
                        argWeight = -1

                    if (argWeight<0 or argWeight>1):
                        print('\nERROR: the weight for \''+argName+'\' is out of bounds or has not been inserted, \nplease change the weight to a value between 0 and 1'+
                              '\nA weight of 0 was assigned to the argument!\n')
                        weights[argName] = 0
                    else:
                        weights[argName] = argWeight

                    """second split of the line into a list of conclusions premises and exceptions"""
                    infoList = re.split(';', info[1])
                    
                    if (len(infoList)!=4):
                        print('\nERROR: wrong syntax for argument \''+argName+'\' in input text, wrong use of \';\', three semicolons need to be instantiated at all times'+
                                   '\nThe argument \''+argName+'\' wasn\'t uploaded!\n')
                    else:

                        """--CONCLUSIONS--
                           separates the list of conclusions from the line being processed, processes it so to have no white space or empty elements in the
                           list of strings and converts this in a list of propositions to be loaded in caes"""
                        conclusions = re.findall('[\w-]*', infoList[0])
                        conclusions = list(filter(lambda x: x!="" and x!=" ", conclusions))
                        if (len(conclusions)!=1):
                            print('\nERROR: too many conclusions were added in \''+argName+'\', only the first will be considered\n')
                        prepConclusions = self.argToProp(conclusions)

                        """--PREMISES---
                           separates the list of premises from the line being processed, processes it so to have no white space or empty elements in the
                           list of strings and converts this in a list of propositions to be loaded in caes"""
                        premises = re.findall('[\w-]*', infoList[1])
                        premises = list(filter(lambda x: x!="" and x!=" ", premises))
                        prepPremises = self.argToProp(premises)

                        """--EXCEPTIONS--
                           separates the list of exceptions from the line being processed, processes it so to have no white space or empty elements in the
                           list of strings and converts this in a list of propositions to be loaded in caes"""
                        exceptions = re.findall('[\w-]*', infoList[2])
                        exceptions = list(filter(lambda x: x!="" and x!=" ", exceptions))
                        prepExceptions = self.argToProp(exceptions)

                        """creates argument and loads it in carneades"""
                        arg = Argument(prepConclusions[0], premises=prepPremises, exceptions=prepExceptions )
                        argset.add_argument(arg, arg_id = argName)
                        print(i+1,'argument loaded successfully!')
						

            """-------------PROOF STANDARDS-------------------"""
			
            """ extracts proof standards from the text"""
            pStandard = re.split(';', section[1])
            pStandard = list(filter(lambda x: x!="" and x!=" ", pStandard))
            
            """creates a list of tuples for all propositions to be assigned a proof standard"""
            proofList = list()
            for i in range(len(pStandard)-1):
                pParts = re.findall('\w*', pStandard[i])
                pParts = list(filter(lambda x: x!="" and x!=" ", pParts))
                if (len(pParts)!=2): 
                    print('ERROR: wrong syntax in Proof Standard section, please insert comma separated lists ending with \';\''+
                          'No proof standard for \''+pParts[0]+'\' was uploaded!')
                else:
                    proofList.append((self.argToProp(pParts)[0], pParts[1]))
                    print('proof standard for \''+pParts[0]+'\' loaded successfully!')
            proofList = list(filter(lambda x: x!="" and x!=" ", proofList))
            ps = ProofStandard(proofList)
			
            """-------------ASSUMPTIONS-----------------------"""
			
            """ extracts assumptions from text"""
            assumptions = re.findall('\w*', section[2])
            assumptions = list(filter(lambda x: x!="" and x!=" ", assumptions))
            assumptions = self.argToProp(assumptions)
...
class ProofStandard(object):
    ...
    def has_standard(self, proposition):
        return (proposition in self.config.keys())
...
class CAES(object):
    ...
    def meets_proof_standard(self, proposition, standard):
        ...
        if standard == 'scintilla':
            ...
            result = self.meets_proof_standard(proposition, 'scintilla') and \
            ...
            diff_exceeds_beta = (mwp - mwc) > self.beta
            ...
            logging.debug("diff between pro and con = {} > beta: {}".\
                          format(mwp-mwc, diff_exceeds_beta))

            result = (exceeds_alpha) and (diff_exceeds_beta)
